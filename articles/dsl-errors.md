...
instead of `dim(x)`, though we will try and suggest the appropriate
...
**Obsolete since 0.4.10, these problems are now included within
[E102](#e102)**
...
Invalid input to special lhs function
[`dim()`](https://rdrr.io/r/base/dim.html). There can be multiple
arguments but there must be at least one. All arguments must be symbols,
and named arguments are not allowed.
...
can accept. You can use `+`, `-`, `(`, `length`, `nrow` and `ncol` but
nothing else. Let us know if you think you should be able to use more.
...
Assignment to reserved words in the monty DSL is not allowed. Currently
the list of reserved words includes `dim` (as this is a special
function) and the index variables: `i`, `j`, `k`, `l`, `i5`, `i6`, `i7`
and `i8`. So
...
side of any expression. Thus `pi` cannot be used as the name of a
variable, but e.g. `pi1` or `pi_1` are allowed.
...
quite strict. If you use `:` it must be the *outermost* operator within
an index, so this is fine:
...
`from:to` form, which we need for the code generation to work, and you
should use bracketing here to be more specify if you really mean
`(a + 1):(b + 1)` or `a:(b + 1)`.
...
arrays. This error is a special case of [`E113`](#e113) but deserves
...
referenced some variable that does not exist within your code. Common
reasons for this error include:

- A spelling mistake: you’ve referenced a variable that is very similar
  to the one that you meant to.
- You forgot to include an equation defining the variable.
- You forgot to include a value in `fixed`.

## `E209`

An assignment or relationship expression for an array did not have a
corresponding call to [`dim()`](https://rdrr.io/r/base/dim.html). We
always need this, even if it looks like we should be able to work out
how long your array is. You probably just need to add a call like
...
You have used a symbol on the lhs of a dim assignment that does not have
a value in fixed.
...
as array dimensions are evaluated before all other expressions (and they
cannot be stochastic).

## `E211`

You have tried to assign dimensions to an array using a value in fixed
that is not scalar.
...
then the value of `d` in `fixed` should be scalar. If you wanted
e.g. `a` to be a two-dimensional array then you could declare dimensions
as
...
where the values of `d1` and `d2` would both be scalar. This makes it
easier to determine the rank of an array while parsing.

## `E212`

In a dim assignment you have used a variable in
[`dim()`](https://rdrr.io/r/base/dim.html) on the rhs that does not has
a dimensions assignment expression itself.
...
Cyclic dependency detected within dim equations. There are a few ways
...
You have tried an assignment or relationship expression for an array
variable (i.e., something with a
...
with no assignments or relationships for other variables in between
(comments are fine). So this is an error:
...
There are no stochastic relationships (with `~`) in your model, and you
have only included assignments (with `<-`). Your model code should have
at least one stochastic relationship.
...
Trying to access an array without using square bracket indexes. For
...
errors because `x[4]` is outside the bounds defined by your `dim`
equation for `x`.
...
errors on the rhs as in `x[] <- a[i]` we loop over the first dimension
of `x` which is defined to have size 5 but `a` only has 4 elements so
`a[5]` is out-of-bounds.
...
Here the `dim` equation for `x` tells us to expect a definition for
`x[5]` but it is not defined by the existing equations for `x`. It could
be possible that you meant to cover it in the existing equations, e.g.
...
or that you meant to include a separate equation defining it (and
possibly other undefined elements of `x`) e.g.
...
An element of an array has multiple definitions (across multiple
equations). This could arise from having defined the entire array more
than once, e.g.
...
An array has some elements defined as an assignment (with `<-`) and some
defined as a stochastic relationship (with `~`).
...
An array variable must be either entirely defined with assignments or
entirely with stochastic relationships.

## `E225`

You are trying to use an array element on the rhs before it has been
defined. A simple example is
...
In the `monty` DSL, equations are evaluated in the order in which they
appear so here it will evaluate `x[1] <- x[2]` first, before `x[2]` has
been defined.
...
Since the equation is evaluated as a loop in order from `i = 1` to
`i = 4`, so first `x[1]` (`i = 1`) will be evaluated but it depends upon
`x[2]` which has yet to be defined.
