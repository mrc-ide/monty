// A simple example showing how to use mcstate's random number generator
...
//
// * include `LinkingTo: cpp11, mcstate` within your DESCRIPTION so that
//   R will find the mcstate headers when compiling (you do not need to
...
cpp11::doubles random_normal(int n, double mu, double sd, int seed) {
  using rng_state_type = mcstate::random::generator<double>;
  auto state = mcstate::random::seed<rng_state_type>(seed);

  cpp11::writable::doubles ret(n);
  for (int i = 0; i < n; ++i) {
    ret[i] = mcstate::random::normal<double>(state, mu, sd);
  }
