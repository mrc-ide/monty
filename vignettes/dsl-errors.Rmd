...
Invalid left hand side of a relationship operator (`~`).  Currently the left hand side must be symbol, though this will be relaxed in future once we support an array syntax.  However, you may not use things like numbers or function calls on the left hand side.
...
Assignments must be unique within the dsl code because makes it straightforward to trace usage through the dependency graph and from this create a gradient function using automatic differentation.
...
A relationship (with `~`) is shaddowing a previous assignment.  So after assigning to a variable you have declared that the same symbol refers to a parameter.
...
An assignment (with `<-`) is shaddowing a previous relationship.
...
Variables are used out of order.  If you are using odin this is a big departure - at the moment you must declare your expressions (assignments and relationships) in order.  However, because we forbid multiple assignment we may relax this in the future, but no existing programmes should be changed.
